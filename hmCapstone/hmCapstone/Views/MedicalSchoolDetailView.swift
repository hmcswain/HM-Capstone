...
// orginal working code below.  can revert back to this if changes dont work
/*
 import SwiftUI
...
struct MedicalSchoolDetailView: View {
  ...
  init() {
    _viewModel = StateObject(wrappedValue: MedicalSchoolDetailsViewModel(userDefaults: .standard))
  }
  ...
  var body: some View {
    NavigationView {
      Form {
        ...
        // Add other fields as in the original view
        Section {
          Button("Save Details") {
            Task {
              await viewModel.saveData()
